...
func TestClassifiedAdPublishedConsumer_AppendsPublishedEntry(t *testing.T) {
	// Given
	setup := newConsumerTestSetup(t)
	require.NoError(t, NewClassifiedAdPublishedConsumer(setup.publicBus, setup.appendHistory))
	occurredAt := time.Date(2026, 7, 31, 12, 0, 0, 0, time.UTC)

	// When
	err := setup.publicBus.Publish(&shared.ClassifiedAdPublished{
		ClassifiedAdID: "ad-1",
	...
	assert.Equal(t, domain.HistoryActionPublished, entries[0].Action())
...
func TestClassifiedAdPublishedConsumer_IgnoresUnexpectedEventType(t *testing.T) {
	// Given
	setup := newConsumerTestSetup(t)
	require.NoError(t, NewClassifiedAdPublishedConsumer(setup.publicBus, setup.appendHistory))

	// When
	err := setup.publicBus.Publish(&mockEvent{eventType: shared.ClassifiedAdPublishedEventType})
